...
之前都在看x86的文档。今天正式回到我们zCore的开发。

直接使用`perf kvm report`似乎能进入一个命令行UI界面，不过没什么用就是了...

```
# perf kvm record -e kvm:* -p 4139
[ perf record: Woken up 14169 times to write data ]
Warning:
Processed 34526770 events and lost 3 chunks!

Check IO/CPU overload!

[ perf record: Captured and wrote 3466.996 MB perf.data.guest (33322683 samples) ]
...
Analyze events for all VMs, all VCPUs:
             ...
             VM-EXIT    Samples  Samples%     Time%    Min Time    Max Time         Avg time 
               ...
               CPUID    9296845    91.72%    15.13%      0.55us    332.16us      0.68us ( +-   0.04% )
       EPT_MISCONFIG     646952     6.38%     5.82%      0.75us    171.70us      3.74us ( +-   0.09% )
      IO_INSTRUCTION      78798     0.78%     1.71%      2.99us    244.64us      9.02us ( +-   0.24% )
       EPT_VIOLATION      34623     0.34%     0.53%      1.10us    212.44us      6.37us ( +-   0.29% )
    PREEMPTION_TIMER      34296     0.34%     0.07%      0.55us     43.64us      0.82us ( +-   0.38% )
                 HLT      34240     0.34%    76.47%      0.83us   1396.32us    929.56us ( +-   0.03% )
  EXTERNAL_INTERRUPT      10416     0.10%     0.28%      0.61us    521.94us     11.22us ( +-   1.28% )
    INTERRUPT_WINDOW         26     0.00%     0.00%      0.80us      6.67us      1.77us ( +-  15.14% )
            MSR_READ         23     0.00%     0.00%      0.72us      5.32us      1.33us ( +-  15.02% )
       EXCEPTION_NMI          3     0.00%     0.00%      5.17us      5.98us      5.68us ( +-   4.50% )

Total Samples:10136222, Total events handled time:41624162.62us.
```

上面是不是说90%以上的vmexit来自CPUID指令？然后75%以上的耗时来自HLT指令，平均耗时1400微秒即1.4毫秒。猜想应该是来自core 0的runtime发现没有任务进入WFI模式。目前的时钟中断频率为4000Hz，间隔应该是0.25毫秒啊，不知道为什么有这么大差距。注意到CPUID指令一次也会耗费0.3毫秒，是不是因为这个呢？

CPUID跟TSC是否有关呢？在获取当前时间的时候，实现如下：
...
Rust内部的实现应该就是直接执行RDTSC指令。看了一下Intel文档，RDTSC和HLT在某种情况下确实会导致VMEXIT。

于是尝试在QEMU里面加上`-vmx-hlt-exit,-vmx-rdtsc-exit`试一下。看起来完全没变化，那要换成`+`？

这次QEMU报了两个warning：

```
qemu-system-x86_64: warning: This feature depends on other features that were not requested: MSR(48EH).vmx-hlt-exit [bit 7]
qemu-system-x86_64: warning: This feature depends on other features that were not requested: MSR(48EH).vmx-rdtsc-exit [bit 12]
```

调整了一下参数，变成：Nehalem,-x2apic,+invtsc,-vmx-hlt-exit,-vmx-rdtsc-exit，但是结果还是一样...

想了一下，CPUID应该和RDTSC没有关系。因为kernel-hal的timer_now每次确实会调用一次RDTSC，但是不会调用CPUID，只会在初始化的时候调用一次。从实验来看也没有RDTSC出现。但是既然这样的话，为什么HLT却能够导致VM exit呢？这让人不太能理解。另一个问题则是CPUID在哪里被调用了这么多次？
...
Analyze events for all VMs, all VCPUs:
             ...
             VM-EXIT    Samples  Samples%     Time%    Min Time    Max Time         Avg time 
               ...
               CPUID   11713262    93.96%    15.21%      0.34us   5364.21us      0.46us ( +-   0.10% )
       EPT_MISCONFIG     646956     5.19%     6.18%      0.58us    127.83us      3.40us ( +-   0.07% )
      IO_INSTRUCTION      78181     0.63%     1.47%      2.25us    170.44us      6.69us ( +-   0.25% )
       EPT_VIOLATION       8172     0.07%     0.14%      0.93us    174.03us      5.90us ( +-   0.96% )
                 HLT       7074     0.06%    76.92%     22.14us  12974.16us   3876.57us ( +-   0.08% )
    PREEMPTION_TIMER       7070     0.06%     0.02%      0.36us     25.45us      0.84us ( +-   1.22% )
  EXTERNAL_INTERRUPT       5156     0.04%     0.07%      0.78us     98.45us      5.05us ( +-   1.32% )
            MSR_READ         17     0.00%     0.00%      0.86us      3.14us      1.56us ( +-   9.63% )
    INTERRUPT_WINDOW         11     0.00%     0.00%      0.74us      1.69us      1.31us ( +-   5.80% )
       EXCEPTION_NMI          1     0.00%     0.00%      5.54us      5.54us      5.54us ( +-   0.00% )

Total Samples:12465900, Total events handled time:35648904.08us.
```

可以看到，HLT的数量下降为1/4，同时平均用时上升为4倍。说明真正和时钟中断挂钩的可能是HLT。想想并不难得出这个结论。

相对的，CPUID的数目则从9M上升为11M，感觉应该正比于总用时。

突发奇想分别看看每个CPU上的情况：

```
Analyze events for all VMs, VCPU 0:

             VM-EXIT    Samples  Samples%     Time%    Min Time    Max Time         Avg time 

               CPUID     388847    95.71%     0.96%      0.00us   5364.21us      0.68us ( +-   2.02% )
       EPT_VIOLATION       7382     1.82%     0.17%      0.00us    141.47us      6.21us ( +-   0.90% )
                 HLT       7074     1.74%    98.80%      0.00us  12974.16us   3876.57us ( +-   0.08% )
      IO_INSTRUCTION       2861     0.70%     0.07%      0.00us    109.57us      6.92us ( +-   1.19% )
  EXTERNAL_INTERRUPT         57     0.01%     0.00%      0.00us     31.92us      5.28us ( +-  13.77% )
            MSR_READ         17     0.00%     0.00%      0.00us      3.14us      1.56us ( +-   9.63% )
    INTERRUPT_WINDOW         11     0.00%     0.00%      0.00us      1.69us      1.31us ( +-   5.80% )
    PREEMPTION_TIMER          8     0.00%     0.00%      0.00us      4.76us      2.38us ( +-  15.26% )

Total Samples:406257, Total events handled time:27754860.83us.

Analyze events for all VMs, VCPU 1:

             VM-EXIT    Samples  Samples%     Time%    Min Time    Max Time         Avg time 

               CPUID   11324415    93.90%    65.30%      0.00us    135.14us      0.46us ( +-   0.02% )
       EPT_MISCONFIG     646956     5.36%    27.89%      0.00us    127.83us      3.40us ( +-   0.07% )
      IO_INSTRUCTION      75320     0.62%     6.38%      0.00us    170.44us      6.68us ( +-   0.25% )
    PREEMPTION_TIMER       7062     0.06%     0.07%      0.00us     25.45us      0.84us ( +-   1.22% )
  EXTERNAL_INTERRUPT       5099     0.04%     0.33%      0.00us     98.45us      5.05us ( +-   1.33% )
       EPT_VIOLATION        790     0.01%     0.03%      0.00us    174.03us      3.01us ( +-   8.04% )
       EXCEPTION_NMI          1     0.00%     0.00%      0.00us      5.54us      5.54us ( +-   0.00% )
```

这个数据非常值得深入研究，不过先休息一会...

## 02/08/22

思考为什么core1上出现了远比core0上更多的CPUID。

看了一下，感觉是我写的模块用了zyr的关中断锁，然后里面居然每次开/关中断都需要通过CPUID获取当前core ID，可谓是完全的无用操作了。所以先换成`spin::Mutex`试试。这样做完之后，core1上的CPUID从11M下降到了2.5M左右。core0上则是比较稳定的366K。我们还需要继续看一下。

> 另外，如何避免每次开/关中断的时候都要通过CPUID获取当前CPU数据结构？
>
> 需要在初始化的时候就把CPUID包进去...

剩下的，我觉得是在正常执行syscall的时候也会用到关中断锁。所以问题的关键可能是优化这个关中断锁。从kernel-sync的实现来看，重点在于维护每个CPU当前的持有的关中断锁的层数，以及第一次持有关中断锁之前中断的打开状态。因此，在push/pop_off的时候需要查看当前core ID来拿到这个数据结构。

关于中断：跟zyr确认了一下目前zCore只要三种情况下开中断：即处理系统调用、PreemptiveSched没有任务WFI还有执行应用程序的时候。

执行应用的时候中断是打开的。依据是在`kernel-hal/common/context.rs`中，注意`set_uspace`在x86_64平台下的表现，将RFLAGS.IF设置为1，于是执行应用之后自然就打开中断了。zCore启动的时候是关闭中断的。这则是需要参考`trapframe-rs`的实现：
...
可以看到一开始就关闭了中断。至于应用进入内核的情况，贾爷提到有两种情况：一是普通中断，这个时候应该是根据中断向量号查IDT，然后IDT里面应该都是中断门（又确认了一下文档），走这个门的话进到handler之后默认关中断，虽然我们目前还没找到IDT的内容是在哪里填充的；二是syscall，这里我们可以找到trapframe-rs中的syscall初始化代码：
...
这里首先是使能syscall指令。然后LStar和SFMask分别表示syscall handler的入口地址和进入syscall handler之后RFLAGS如何变化。总之这里我们通过设置使得RFLAGS.IF执行syscall之后被清空，也就是关中断了。

重新看一下IDT的初始化。首先每个IDT entry应该是16字节，可能是陷入门或者中断门。关键点在于找到`__vectors`这个符号。

在trapframe-rs的build.rs中找到如下代码：
...
可以看到`__vectors`每个表项是一个函数指针vectorx，该函数往栈上push error code然后跳转到`__alltraps`。

IDT的初始化也找到了，是来自x86_64里面的`InterruptDescriptorTable::new`，进而到最终的`EntryOption::minimal`，将每个IDT表项的gate type设置为0b1110，也就是一个中断门。而在后面也并没有将它们改成陷入门。这个算是明白了QAQ

接下来再来看看TLS的问题。首先FS/GS应该仍然是段选择子，指向GDT里面的段描述符，但是应该只有其中的基地址是有用的（limit、权限等应该没用）。这些基地址会作为某些per CPU数据结构的起始地址。但是这样会比较麻烦，于是AMD新增了（但是目前应该是x86_64通用的）三个MSR，分别是GSBase、KernelGSBase和FSBase让我们可以直接通过这些MSR指定TLS的起始地址了。

在trapframe-rs的gdt的初始化中，貌似可以看到：
...
这个时候GsBase里面是TSS的起始地址。每个核都应该有自己的GsBase和TSS，因为`trapframe::init`是在每个core上都被调用一次的。

上面的猜想是否成立？x86文档vol4第二章可以看到MSR分成不同的scope，包括thread/core/module/die/package。scope的含义是在scope内的处理器共享一个MSR（在通过RDMSR/WRMSR读写MSR的时候）。考据了一下，之前看到的一大堆MSR的scope都是thread，于是这是一个很好的TLS基础。
...
x86_64里面我们找到TSS的定义。那么看起来在这里面的保留字段里面加上一个cpuid是完全可能的。接下来我们可以找找这些空间是否已经被占用了。在x86_64的syscall.S中，可以看到，当执行syscall指令的时候，进入syscall_entry函数。首先执行swapgs指令交换GsBase和KernelGsBase，也就是目前的GsBase应该指向kernel的TSS了。然后将应用rsp保存到TSS.sp1（这里的sp0/1指的是上面的privileged_stack_table），从TSS.sp0加载kernel的rsp。后面的`pop rsp`应该属于x86限定技巧，后面我们应该能找到对应。抛去这个不谈，trapframe上一共需要压入22个8字节内容，按照压入的顺序是error_code(=0)、trap_num(=0x100=256)、fsbase、gsbase(这两个后面再修改的)、RFLAGS、RIP还有16个通用寄存器。等到通用寄存器压入完毕，再通过rdmsr指令读取fsbase和gsbase这两个MSR并在栈上对应位置修改。后面是恢复kernel的被调用者保存寄存器，虽然有点没看懂。

虽然还没有完全看懂，但我觉得我想的做法理论上是可行的，一会试试。

直接把kernel-sync替换掉不太行，core1初始化的时候（应该是irq::apic）那里会panic...?明天再研究下。

## 03/08/22

感觉是堆分配器用了关中断锁，目前的实现是core1需要通过堆分配来创建tss，至少在这之后我们才能修改里面的cpuid。所以这个就很麻烦...一种思路可以是堆分配器用原先的锁，然后其他部分有新的锁...不过这个方案做起来有点麻烦。有没有什么更优雅的办法？感觉可以试试在trapframe-rs里面自启动！试了一下，好像成功了233333

看起来成效十分显著，CPUID基本上被干掉了：
...
Analyze events for all VMs, all VCPUs:
       ...
       EPT_MISCONFIG     646948    78.29%     7.68%      0.42us    331.89us      2.90us ( +-   0.09% )
      IO_INSTRUCTION      81484     9.86%     1.67%      2.11us    146.81us      5.01us ( +-   0.31% )
               CPUID      74739     9.04%     0.20%      0.33us     31.84us      0.66us ( +-   1.05% )
  EXTERNAL_INTERRUPT       6221     0.75%     0.12%      0.83us    247.44us      4.89us ( +-   2.42% )
       EPT_VIOLATION       5787     0.70%     0.17%      0.78us    105.35us      7.08us ( +-   1.36% )
                 HLT       5585     0.68%    90.14%    431.20us  13555.20us   3940.14us ( +-   0.09% )
    PREEMPTION_TIMER       5573     0.67%     0.01%      0.34us     90.65us      0.62us ( +-   3.25% )
            MSR_READ         12     0.00%     0.00%      0.64us      2.32us      1.03us ( +-  14.78% )
       EXCEPTION_NMI          3     0.00%     0.00%      5.39us      6.77us      5.90us ( +-   7.40% )
    INTERRUPT_WINDOW          3     0.00%     0.00%      0.65us      1.19us      0.86us ( +-  19.76% )

Total Samples:826355, Total events handled time:24412083.79us.
      
Analyze events for all VMs, VCPU 0:

             VM-EXIT    Samples  Samples%     Time%    Min Time    Max Time         Avg time 

               CPUID      74666    83.48%     0.22%      0.00us     31.84us      0.66us ( +-   1.05% )
       EPT_VIOLATION       5756     6.44%     0.18%      0.00us    105.35us      7.10us ( +-   1.36% )
                 HLT       5585     6.24%    99.51%      0.00us  13555.20us   3940.14us ( +-   0.09% )
      IO_INSTRUCTION       2568     2.87%     0.06%      0.00us    101.45us      5.32us ( +-   1.84% )
  EXTERNAL_INTERRUPT        852     0.95%     0.02%      0.00us     56.97us      4.68us ( +-   2.99% )
            MSR_READ         12     0.01%     0.00%      0.00us      2.32us      1.03us ( +-  14.78% )
    INTERRUPT_WINDOW          3     0.00%     0.00%      0.00us      1.19us      0.86us ( +-  19.76% )
    PREEMPTION_TIMER          3     0.00%     0.00%      0.00us     90.65us     31.01us ( +-  96.18% )

Total Samples:89445, Total events handled time:22113370.09us.

Analyze events for all VMs, VCPU 1:

             VM-EXIT    Samples  Samples%     Time%    Min Time    Max Time         Avg time 

       EPT_MISCONFIG     646948    87.79%    81.52%      0.00us    331.89us      2.90us ( +-   0.09% )
      IO_INSTRUCTION      78916    10.71%    17.17%      0.00us    146.81us      5.00us ( +-   0.32% )
    PREEMPTION_TIMER       5570     0.76%     0.15%      0.00us     21.15us      0.60us ( +-   2.00% )
  EXTERNAL_INTERRUPT       5369     0.73%     1.15%      0.00us    247.44us      4.93us ( +-   2.75% )
               CPUID         73     0.01%     0.00%      0.00us     16.64us      1.07us ( +-  24.43% )
       EPT_VIOLATION         31     0.00%     0.00%      0.00us     22.43us      3.37us ( +-  26.22% )
       EXCEPTION_NMI          3     0.00%     0.00%      0.00us      6.77us      5.90us ( +-   7.40% )

Total Samples:736910, Total events handled time:2298713.70us.

```

接下来简单统计一下`ls libc-test/src/math`会有多少次syscall，看起来应该是在1800左右。

总CPUID数目从10M左右下降到74K，变为原来的0.75%，效果超级拔群。注意kernel_hal里面的cpuid实现还没有替换过。

感觉这次总算是kvm比较快了？固定时钟中断频率1KHz，打开kvm用时8.49s，关闭kvm用时10.96s，终于实现了超越哈哈。

接下来可以尝试优化一下zCore中断架构的问题了，其实可能并不是个问题？

目前的中断架构是：内核态大多数时候关闭中断。只有当执行syscall和在runtime里面没有任务WFI的时候才打开中断。考虑目前的远程调用框架，core0提交请求后，立刻停止对任务队列的轮询而是开始进入WFI状态直到下一个时钟中断，而这是在core1处理完毕之前发生的。

先不考虑我们的拓展。原版的设计相当于将中断的优先级降至最低，只有当所有的thread任务都处于等待状态的时候才打开中断。不过在syscall的时候打开中断一定程度上缓解了这个问题。

## 04/08/22

问题在于，原版的core0调度器应该只占据很小的一环。但是一旦没有任务就会死在调度循环里面。那么的话，我们其实可以在进入调度循环的时候打开中断，等找到了任务之后再关闭中断。毕竟反正没有任务，CPU没办法做有用的事情，我们暂时也不必考虑功耗。既然用上关中断锁，我觉得对于可以把中断的优先级再拉高一点。

这个时候打开中断是否可行呢？一种没有什么负面影响的操作是在检查到没有任务的时候，先打开中断，等待一段极短的时间再关闭中断。看起来不太行，不打开kvm直接拉到40s左右了。把等待的100K的cycle缩小为100个cycle之后变成11.36s，但仍然和我们预期的效果相差比较远。打开kvm之后则是6.2s，倒是比之前要快上一些。有没有一种可能就是系统调用本身就需要耗时这么久所以没办法更快了？跟之前对比了一下：

```
[  6.772592 INFO  1 0:0 linux_syscall::async_syscall] task 543 dropped
[  6.773457 INFO  1 0:0 linux_syscall::async_syscall] wakeup client!
[  6.774187 INFO  1 0:0 linux_syscall::async_syscall] queue op completed, before adding new req
[  6.775042 INFO  1 0:0 linux_syscall::async_syscall] after adding new req
[  6.775820 INFO  1 0:0 linux_syscall::async_syscall] select task id: 544
[  6.776595 INFO  1 0:0 linux_syscall::file::stat] fstatat: dirfd=FileDesc(-100), path="libc-test/src/math/erff.exe", stat_ptr=0x317c00, flags=SYMLINK_NOFOLLOW
[  6.778529 INFO  1 0:0 linux_syscall] <= Ok(0)
```

注意这是一个系统调用循环。第一行表示一个syscall请求做完了被drop掉；第二行表示成功调用core0提供的callback函数唤醒相应的task；第三行则已经成功取到core0新提交的请求了。二三行之间仅有0.7ms的延迟，其中还有应用程序执行和内核必要处理的时间，已经和中断解耦合了。然而整体上是比之前要慢的，因为我们用virtio块设备替换掉了ramfs。打开kvm，再优化了一下core0 runtime的cpuid实现甚至变成5.16s。看一下当前vmexit数据：

```
root@shinbokuow-SEi:/home/shinbokuow/Desktop# perf kvm stat report --event vmexit
...
Analyze events for all VMs, all VCPUs:
       ...
       EPT_MISCONFIG     646956    85.39%    85.87%      0.50us   2109.09us      5.42us ( +-   0.13% )
      IO_INSTRUCTION      78779    10.40%    11.49%      2.46us    174.32us      5.95us ( +-   0.23% )
    PREEMPTION_TIMER      10994     1.45%     0.39%      0.34us    116.50us      1.45us ( +-   1.77% )
  EXTERNAL_INTERRUPT      10856     1.43%     1.53%      0.64us   4018.27us      5.77us ( +-   7.45% )
       EPT_VIOLATION       5710     0.75%     0.63%      1.00us    187.48us      4.51us ( +-   1.87% )
    INTERRUPT_WINDOW       4238     0.56%     0.09%      0.42us     20.02us      0.84us ( +-   1.18% )
            MSR_READ        139     0.02%     0.00%      0.86us      8.88us      1.36us ( +-   6.01% )
       EXCEPTION_NMI          4     0.00%     0.00%      5.89us     49.02us     16.90us ( +-  63.37% )

Total Samples:757676, Total events handled time:4083770.39us.

root@shinbokuow-SEi:/home/shinbokuow/Desktop# perf kvm stat report --event vmexit --vcpu 0
...
Analyze events for all VMs, VCPU 0:
  ...
  EXTERNAL_INTERRUPT       6203    25.23%    38.77%      0.00us   4018.27us      5.94us ( +-  12.50% )
       EPT_VIOLATION       5671    23.07%    27.00%      0.00us    187.48us      4.52us ( +-   1.87% )
    PREEMPTION_TIMER       5497    22.36%    11.87%      0.00us    116.50us      2.05us ( +-   2.38% )
    INTERRUPT_WINDOW       4238    17.24%     3.76%      0.00us     20.02us      0.84us ( +-   1.18% )
      IO_INSTRUCTION       2833    11.52%    18.34%      0.00us     84.72us      6.15us ( +-   1.13% )
            MSR_READ        139     0.57%     0.20%      0.00us      8.88us      1.36us ( +-   6.01% )
       EXCEPTION_NMI          2     0.01%     0.06%      0.00us     49.02us     27.59us ( +-  77.65% )

Total Samples:24583, Total events handled time:95013.59us.

root@shinbokuow-SEi:/home/shinbokuow/Desktop# perf kvm stat report --event vmexit --vcpu 1
...
Analyze events for all VMs, VCPU 1:
       ...
       EPT_MISCONFIG     646956    88.25%    87.91%      0.00us   2109.09us      5.42us ( +-   0.13% )
      IO_INSTRUCTION      75946    10.36%    11.32%      0.00us    174.32us      5.95us ( +-   0.23% )
    PREEMPTION_TIMER       5497     0.75%     0.12%      0.00us     24.96us      0.85us ( +-   1.32% )
  EXTERNAL_INTERRUPT       4653     0.63%     0.65%      0.00us    395.61us      5.53us ( +-   2.81% )
       EPT_VIOLATION         39     0.01%     0.00%      0.00us      8.27us      2.67us ( +-  12.67% )
       EXCEPTION_NMI          2     0.00%     0.00%      0.00us      6.52us      6.20us ( +-   5.10% )

Total Samples:733093, Total events handled time:3988756.80us.
```

感觉还是相当不错的，CPUID和HLT被完全干掉了，现在vmexit的来源是page fault和各种中断了，想必是可以接受的。

在我们的设计中，目前core0做的事情可以被看做一个应用程序异步模块，里面的任务都是应用程序的线程Future。对于一个异步模块来说，会有一种可能就是它正在执行的所有任务都在等待其他模块而被阻塞，那么这个时候我们应该把这个模块换成其他模块，也就是需要有一个更加顶层的调度器。不过这个设计有些复杂了，要到后面再说。

接下来需要搞一下virtio中断的问题，甚至可能需要重塑zCore的异步I/O框架。

找一下之前的基础设施代码。`zCore/drivers/pci.rs`里面已经有一个enable函数，这个貌似只支持MSI而不是MSI-X。

## 05/08/22

不太清楚PCI是如何连接到APIC上面的，为此可能还需要补充一下APIC相关的知识。

[[1]](https://www.kernel.org/doc/html/latest/x86/i386/IO-APIC.html)这里提到I/O APIC才能实现中断到多个CPU的转发。如果没有它的话，中断默认只会被第一个CPU收到。

## 09/08/22

Lluis提到根据kvm的数据显示我的实现比原版慢0.5倍左右（从每个EPT_MISCONFIG事件的耗时来看，据说是来自于MMIO寄存器写入），那么我们跑一下看看是不是这样。应该是没有这么慢的。我的实现：7.18s；原版：7.41s，也就是说总体上应该是差不多的。

再重新跑一下kvm看看EPT_MISCONFIG用时(average time cost of one EPT_MISCONFIG event)的比较，这次我们尽量多跑几次。

原版(original zCore)：5.45us(4.13s);3.87us(3.13s);3.57us(2.82s);3.64us(2.92s);3.62us(2.96s);3.67us(2.95s)

我的实现(my implementation)：3.66us(2.93s);3.63us(2.89s);3.56us(2.81s);3.61us(2.86s);3.79us(3.02s);3.71us(2.94s)3.64us(2.89s);3.70us(2.94s);

目前得出的结论是无论对于我的实现还是原版，有非常低的概率EPT_MISCONFIG的单次用时显著提升（从3.6us变为5.4us），从而导致总用时飙升。但大多数情况下都不是这样。

那我们继续来看MSI-X....

目前在kernel-hal中的x86/interrupt.rs中可以看到有`register_irq_handler`和`msi_register_handler`这两个函数。这样看起来的话可能我们还需要一个`msix_register_handler`，因为MSI-X和MSI是不同的。
...
这里`all_irq().first_unwrap()`会返回一个`dyn drivers::scheme::IrqScheme`。同时在drivers的x86_64下的irq驱动下面，只为Apic实现了IrqScheme，而Apic看起来是同时包含local APIC和IO APIC的。
...
从一些 相关的方法可以看出，IoApicList可以通过一个gsi来找到一个IoApic，在register_handler中就用到了这个过程。但看起来主要还是在manager_ioapic里面进行注册并拿到分配的vector，然后在IoApic中进行更新。而msi_register_handler最终也是注册到manager_ioapic中，由此我们才想msi-x应该也同样是注册到manager_ioapic中。

这里还需要注意，IoApic的内部实现用到了x2apic crate中的实现，如果我们禁用了x2apic的话那这个crate还能正常使用吗？
...
那么中断的处理流程是怎样的呢（现在我比较怀疑在x86上没有正经在使用的中断）？在loader中可以看到这样的代码：
...
但是在kernel-hal里面能够找到另一段代码，这个trap_handler是trapframe-rs依赖的外部接口：
...
这两段代码似乎在不同的情形下均会被用到（参考trapframe-rs中的x86_64.svg），我们暂且不去深究。重点是它们对于中断的处理逻辑都是一致的，即主要是kernel-hal里面的handler_irq(vector)。handler_irq应该来自Apic：
...
也就是根据vector判断中断来自local APIC还是I/O APIC。local APIC的范围应该是[240,256)，其余则是来自I/O APIC。这些handler需要预先进行注册。现在最麻烦的是，zCore/x86_64上似乎没有MSI/MSI-X的现成例子可以用...

Lluis帮我debug了一下目前的vmexit的来源：

1. async版本的exit syscall出现问题导致一直在不断的exit（看起来是个大问题）；
2. EPT_MISCONFIG来自virtio块设备，增加块缓存容量之后可以大大降低EPT_MISCONFIG，但是可能由于块缓存的实现问题总体速度反而慢了很多
3. IO_INSTRUCTION来自于串口输出
4. 还需要弄清楚EXTERNAL_INTERRUPT和PREEMPTION_TIMER的实际频率是否与期望频率相符

将cache大小从0x100调整为0x1000，原版耗时为23.06s，此时RAM总大小2GB，内核堆分配1GB。如果每个块大小为512字节的话，0x100个块总大小是128KB，0x1000的话总大小2MB。BlockCache来自rcore-fs/dev，看了一下是线性时间的，确实cache容量上来之后有可能成为瓶颈了。所以我们姑且还是改回原先的0x100加上内核堆的16MB吧，后面如果有需要的话再优化BlockCache。改回来之后，原版变成了6.2s。

那么我们再来考虑一下为什么我们的实现中会不断调用sys_exit。看一下sys_exit的实现，在linux-object/thread.rs中：
...
在我们的实现中if条件总是为true，导致进程退出失败。debug发现退出的时候先是调用了exit_group这个syscall：
...
这个syscall返回Err其实是正常的。参考原版zCore的输出：

```
[  6.555611 INFO  0 0:0 linux_syscall] pid: 1045 syscall: num=231, args=[0, 0, 0, 0, 0, 0]
[  6.556402 INFO  0 0:0 linux_syscall::task] exit_group: code=0
[  6.557475 INFO  0 0:0 linux_syscall] <= Err(ENOSYS)
[  6.557981 INFO  0 0:0 linux_object::process] Received signal: SIGNALED | TASK_TERMINATED | JOB_TERMINATED | PROCESS_TERMINATED | THREAD_TERMINATED | VMO_ZERO_CHILDREN
[  6.559187 INFO  0 0:0 linux_syscall] <= Ok(1045)
[  6.559738 INFO  0 0:0 linux_syscall] pid: 1028 syscall: num=61, args=[ffffffffffffffff, 31793c, 3, 0, 0, 0]
[  6.560835 INFO  0 0:0 linux_syscall::task] wait4: target=AnyChild, wstatus=0x31793c, options=NOHANG | STOPPED
[  6.561858 INFO  0 0:0 linux_syscall] <= Err(ECHILD)
[  6.562410 WARN  0 0:0 zcore_loader::linux] page fault from user mode @ 0x296bdc(WRITE | USER), pid=1028
```

可以看到这里涉及到zCore的信号处理机制了。在fork子进程的时候（linux-object/process/fork_from）
...
当子进程的signal发生变化的时候会调用这里传进去的callback方法，如果子进程已经结束那么打印子进程的signal并通过信号的方式通知父进程有一个子进程结束了。但相当遗憾的是我的实现中并没有打印那些signal，说明这个callback没被调用？

注意，`kernel_hal::thread::set_current_thread`用到了当前的core ID，但是它们应该只会在core 0上被调用。

信号相关的方法基本上来自KObject，这是一个trait。所有的内核对象都会实现这个trait。每个内核对象都有一个名为KObjectBase的成员，在实现KObject接口的时候发挥重要作用。应该是在signal_change的时候会调用signal_callback。

```
[ 12.232339 INFO  0 0:0 linux_syscall] pid: 1045 syscall: num=231, args=[0, 0, 0, 0, 0, 0]
[ 12.233321 INFO  0 0:0 linux_syscall::task] exit_group: code=0
[ 12.234003 INFO  0 0:0 linux_syscall] <= Err(ENOSYS)
[ 12.234548 INFO  0 0:0 zcore_loader::linux] exit run_user
[ 12.235079 INFO  0 0:0 linux_object::process] Received signal: SIGNALED | TASK_TERMINATED | JOB_TERMINATED | PROCESS_TERMINATED | THREAD_TERMINATED | VMO_ZERO_CHILDREN
[ 12.236589 INFO  0 0:0 linux_syscall] <= Ok(1045)
[ 12.237377 INFO  0 0:0 linux_syscall] pid: 1028 syscall: num=61, args=[ffffffffffffffff, 31793c, 3, 0, 0, 0]
[ 12.238789 INFO  0 0:0 linux_syscall::task] wait4: target=AnyChild, wstatus=0x31793c, options=NOHANG | STOPPED
[ 12.240179 INFO  0 0:0 linux_syscall] <= Err(ECHILD)
[ 12.240961 WARN  0 0:0 zcore_loader::linux] page fault from user mode @ 0x296bdc(WRITE | USER), pid=1028
```

可以看到，在没有新的syscall的情况下，在thread的run_user Future已经退出的情况下，触发了signal_callback。大致的过程是：在exit_group syscall中，首先是`Process::stop`，然后是对于Process内的每个Thread调用`Thread::stop`，这个时候handle_user_trap将返回一个`Err(ENOSYS)`，于是进而将触发run_user里面的`thread.exit_linux`（实现在linux-object/thread中找到），在里面应该会调用`CurrentThread::exit`。做完之后run_user的一轮循环就完成了。下一轮中检查ThreadState发现已经dying，那么这个时候就可以通过break退出循环了。再经过kernel-hal的设置之后，整个run_user退出。忽然发现：
...
run_user闭包里面是包裹着一个CurrentThread的，所以当回收闭包的时候，这个函数会被调用，最终是调用`Thread::terminate`，即：
